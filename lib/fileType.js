...
export function removeFolderFromTree(tree, folderId) {
  return tree
    .filter((folder) => folder.id !== folderId)
    .map((folder) => {
      if (!Array.isArray(folder.children)) return folder;
...
export function addFolderToTree(tree, parentId, folder) {
  return tree.map((treeFolder) => {
    if (treeFolder.id === parentId) {
      const children = Array.isArray(treeFolder.children)
        ? [...treeFolder.children, folder].sort((a, b) =>
    ...
    if (Array.isArray(treeFolder.children)) {
      return {
        ...treeFolder,
        children: addFolderToTree(treeFolder.children, parentId, folder),
      };
    }
...
export function renameFolderInTree(tree, folderId, name) {
  return tree.map((folder) => {
    ...
    if (Array.isArray(renamedFolder.children)) {
      return {
        ...renamedFolder,
        children: renameFolderInTree(renamedFolder.children, folderId, name),
      };
    }
